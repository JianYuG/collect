...
/**
 * 获取当前已画的有效点位数
 * OL Draw 内部草稿包含鼠标跟随点，实际点位数 = 坐标数 - 1
 */
function getDrawnPointCount() {
  if (!drawInteraction) return 0
  try {
    const sketchFeature = drawInteraction.getOverlay().getSource().getFeatures()[0]
    if (!sketchFeature) return 0
    const geom = sketchFeature.getGeometry()
    if (!geom) return 0
    const gType = geom.getType()
    if (gType === 'LineString') {
      return Math.max(0, geom.getCoordinates().length - 1)
...
function bindContextMenu() {
  ...
  // ★ capture 阶段拦截右键 pointerdown，在 OL 把该点加入草稿之前记录点位数并阻止事件传播
  ...
  _contextHandler = (e) => {
    ...
    if (count < getMinPoints()) {
      ...
      ElMessage.warning(`点位不足，绘制40线需要至少 ${need} 个点，请继续绘制`.replace('40线', typeName))
      return
    }
